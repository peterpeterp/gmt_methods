...
for mod_style in ['model','runs']:
	for av_style in ['year','20year']:
		for preind_name,preind_period in zip(['1861-1880','1850-1900'],[[1861,1880],[1850,1900]]):
			...
			for scenario in ['rcp85']:
				for level in gmt_qu.level:
					for x_method in styles:
						...
						plt.savefig('plots/details/quantiles_'+x_method+'_'+str(level)+'_'+av_style+'_'+preind_name+'.png')

			# conversion table
			conversion_table=open('tables/conversion_table_'+av_style+'_'+preind_name+'.txt','w')
			...
			conversion_table=open('tables/conversion_table_precise_'+av_style+'_'+preind_name+'.txt','w')
			conversion_table.write('\t'.join([' ']+['gmt_ar5','gmt_sat','gmt_millar','gmt_bm']))
			for x_method in ['gmt_ar5','gmt_sat','gmt_millar','gmt_bm']:
				conversion_table.write('\n'+x_method+'\t')
				for y_method in ['gmt_ar5','gmt_sat','gmt_millar','gmt_bm']:
			...
			conversion_table=open('tables/conversion_table_SI_'+av_style+'_'+preind_name+'.txt','w')
			...
			ds.write_nc('data/gmt_plot_ready_'+av_style+'_'+preind_name+'.nc', mode='w')

			ds=da.Dataset({'gmt_qu':gmt_qu})
			ds.write_nc('data/gmt_quantiles_'+av_style+'_'+preind_name+'.nc', mode='w')
