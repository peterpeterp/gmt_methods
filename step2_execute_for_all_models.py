	...
	if len(glob.glob('sftof/'+model+'.nc'))!=0:
		...
		for scenario in ['rcp85']:
			if len(glob.glob('data_models/'+model+'_'+run+'/*'+scenario+'*.nc'))==3:
				tas=glob.glob('data_models/'+model+'_'+run+'/tas_'+scenario+'.nc')[0]
				tos=glob.glob('data_models/'+model+'_'+run+'/tos_'+scenario+'.nc')[0]
				sic=glob.glob('data_models/'+model+'_'+run+'/sic_'+scenario+'.nc')[0]
