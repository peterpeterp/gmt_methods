...
slr=pd.read_csv('data/slr_2100_temperature_levels.csv')
...
for time_slice in [3,1,2]:
    ...
    ax[0].fill_between(cmip5_dict['ACCESS1-0']['TXx']._distributions['global']['pdf']['xaxis'],upper,lower,color=time_slice_dict[time_slice]['light_color'],alpha=0.25)

for time_slice in [3,1,2]:
    ax[0].plot(all_cmip5._distributions['global']['pdf']['xaxis'],cmip5_dict[model]['TXx']._distributions['global']['pdf'][time_slice_dict[time_slice]['name']],linewidth=1.5,color=time_slice_dict[time_slice]['color'],label=time_slice_dict[time_slice]['longname'])
...
for time_slice in [3,1,2]:
    y=np.array(slr[str(time_slice_dict[time_slice]['wlvl'])])
    qu=np.nanpercentile(y,[5,25,50,75,95])
    x=time_slice_dict[time_slice]['pos']
    #ax[1].plot([x,x],[qu[0],qu[4]],lw=2,color=time_slice_dict[time_slice]['color'])
    ax[1].fill_between([x-0.03,x+0.03],[qu[1],qu[1]],[qu[3],qu[3]],lw=2,color=time_slice_dict[time_slice]['color'])
...
ax[1].set_xticks([time_slice_dict[time_slice]['pos'] for time_slice in [3,1,2]])
ax[1].set_xticklabels([time_slice_dict[time_slice]['longname'] for time_slice in [3,1,2]])
ax[1].set_ylabel('Sea level rise in 2100 [m]')
#ax[1].set_xlabel('$\mathregular{GMT}$ $\mathregular{[^\circ C]}$')
...
for time_slice in ['1.46','1.59','1.7']:
    ...
    #plt.plot(hist[1][1:]-0.005,hist[0],color=time_slice_dict[time_slice]['color'],label=time_slice_dict[time_slice]['longname'])
...
#     plt.plot(all_cmip5._distributions['global']['pdf']['xaxis'],cmip5_dict['ACCESS1-0']['TXx']._distributions['global']['pdf'][time_slice_dict[time_slice]['name']],linewidth=1.5,color=time_slice_dict[time_slice]['color'],label=time_slice_dict[time_slice]['longname'])
#     print time_slice_dict[time_slice]['longname']
#
