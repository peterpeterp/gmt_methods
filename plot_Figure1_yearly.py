...
for mod_style in ['model','runs']:
	for av_style in ['year']:
		for preind_name in ['1861-1880','1850-1900']:
			...
			gmt_qu=da.read_nc('data/gmt_quantiles_'+av_style+'_'+mod_style+'_'+preind_name+'.nc')['gmt_qu']
			...
			for method in ['gmt_millar','gmt_bm','gmt_sat']:
				tmp=plot_dict[method]
				yy=gmt_qu[scenario,'gmt_ar5',method,1.5,'mean']
				ax[0].plot([gmt_qu[scenario,'gmt_ar5',method,1.5,str(0)],gmt_qu[scenario,'gmt_ar5',method,1.5,str(100)]],[tmp['pos'],tmp['pos']],color=tmp['color'])
				ax[0].fill_between([gmt_qu[scenario,'gmt_ar5',method,1.5,str(1/6.*100)],gmt_qu[scenario,'gmt_ar5',method,1.5,str(5/6.*100)]],[tmp['pos']-0.02,tmp['pos']-0.02],[tmp['pos']+0.02,tmp['pos']+0.02],color=tmp['color'])
				...
				' ('+str(round(gmt_qu[scenario,'gmt_ar5',method,1.5,str(0)],2))+'-'+str(round(gmt_qu[scenario,'gmt_ar5',method,1.5,str(100)],2))+')')
