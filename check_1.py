...
for style,var,title in zip(['xax','xax','had4','had4'],['air','gmt','air','gmt'],['SAT unmasked','Blended air/sea temperature, unmasked, temperature anomalies, variable ice','SAT masked','HadCRUT4 emulation']):
    ngood=0
    ntot=0
    plt.close()
    fig,ax=plt.subplots(nrows=3,ncols=2,figsize=(7,12))
    l_styles = ['-','--','-.',':']
    m_styles = ['','.','o','^','*']
    ...
    for model_run,(marker,linestyle,color) in zip(sorted(gmt.model_run),itertools.product(m_styles,l_styles, colormap)):
        if np.isfinite(np.nanmean(gmt['xax','rcp85',model_run,'gmt',:].values)):
            ...
            if np.nanmean(np.abs(perc_diff))<0.001:
                ...
                ngood+=1
            elif np.nanmean(np.abs(perc_diff))<0.1:
